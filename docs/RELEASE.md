...
1. Ubuntu, Windows and macOS on CPython 3.11-3.13.
2. Wheel and source distribution built by the pinned toolchain.
3. Full unit suite against the installed wheel.
4. Canonical and legacy identity compatibility.
5. Functional probe with intentional counter-example preserved.
6. Installed CLI smoke outside checkout.
7. Tests executed from the complete source distribution.
8. SHA-256 checksums and CycloneDX 1.6 SBOM.
9. Verified GitHub/Sigstore SLSA provenance on the approved wheel.
10. Consumer/redirect review and explicit publication decision.
